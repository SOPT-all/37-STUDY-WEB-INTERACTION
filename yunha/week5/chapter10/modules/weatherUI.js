...
export class WeatherUI {
  ...
  toggleElementVisibility(elementName, isShow, duration = 0) {
    ...
    if (duration > 0) {
      setTimeout(
        () => this.setElementStyle(elementName, "opacity", "0"),
        duration
      );
    }
  }
  ...
  updateTemperatures(main) {
    ...
    this.updateElementContent(
      ELEMENT_NAMES.TEMP_MAX,
      Math.round(main.temp_max)
    );
    this.updateElementContent(
      ELEMENT_NAMES.TEMP_MIN,
  ...
  updateWeatherIcon(iconCode) {
    this.setElementStyle(
      ELEMENT_NAMES.WEATHER_ICON,
      "backgroundImage",
      `url(../public/img/freesvg/weather/icn_${iconCode}.svg)`
    );
  }
