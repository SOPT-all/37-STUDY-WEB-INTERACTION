...
export class WeatherClassifier {
  ...
  getWeatherClass(weatherId) {
    const range = this.weatherRanges.find(
      (range) => weatherId >= range.min && weatherId <= range.max
    );
...
export class Temperature {
  ...
  static convertTemperatures(weatherData, toUnit) {
    ...
    weatherData.wind.speed =
      toUnit === UNITS.METRIC
        ? Number(this.mphToMs(windSpeed)).toFixed(2)
        : Number(this.msToMph(windSpeed)).toFixed(2);
