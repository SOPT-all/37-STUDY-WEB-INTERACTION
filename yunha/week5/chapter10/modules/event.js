...
export class EventHandler {
  ...
  handleOutsideClick(event) {
    if (
      !this.app.$elements.searchInput.contains(event.target) &&
      !this.app.$elements.cityList.contains(event.target)
    ) {
  ...
  toggleTemperature(event) {
    ...
    if (cachedData) {
      const convertedData = Temperature.convertTemperatures(
        cachedData,
        newUnit
      );
