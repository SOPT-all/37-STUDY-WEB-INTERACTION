...
// 사용
document.querySelector('.modulable .js-calculator').addEventListener('click', (event) => renderCalcResult(event, options));

function renderCalcResult(event, options) {
  const $modulable = event.target.closest('.modulable');
  const $result = $modulable.querySelector('.result');
  const { price = 1200, discount = 0.2, tax = 0.08 } = options;
  $result.textContent = calcResult(price, discount, tax);
}
