...
/**
* DOMContentLoaded - HTML DOM 트리가 렌더링이 완료 된 다음 , 추가적인 스크립트가 다운로드
되고 실행될 때 발생합니다.
* @see
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .querySelector(".reusable .js-dateTime")
    .addEventListener("click", (event) => renderDateTime(event));
  document
    .querySelector(".modulable .js-calculator")
    .addEventListener("click", renderCalcResult);
  ...
  document
    .querySelector(".maintainable .js-update")
    .addEventListener("click", (e) => {
      handleUpdateClick(e, userInfo);
    });
});

/**
 * 함수의 재사용성 - form 유효성 검사
 */
...
function renderDateTime(event) {
  const $reusable = event.target.closest(".reusable");
  const $result = $reusable.querySelector(".result");
...
/**
 * 함수의 모듈성 - 세금 반영된 상품 계산기
 * 10개 이상 구매시 할인율 20% 적용
 * 세금 8%
 * 각 1200원
 */
...
function renderCalcResult(event) {
  ...
  const { price, discount, tax } = options;
...
function updateUserInfo(currentInfo, updateInfo) {
  ...
  Object.entries(updateInfo).forEach(([key, newValue]) => {
    ...
    if (
      !(key in updatedUserInfo) ||
      typeof newValue !== typeof updatedUserInfo[key] ||
